	...
	*********************************************************************************************************************************************/
	...
	this.unload = function() {
		...
		if (window.dhtmlXFormLs[this._rId]) {
			for (var q=0; q<window.dhtmlXFormLs[this._rId].inps.lenght; q++) window.dhtmlXFormLs[this._rId].inps[q] = null;
			for (var a in window.dhtmlXFormLs[this._rId].vals) window.dhtmlXFormLs[this._rId].vals[a] = null;
			for (var a in window.dhtmlXFormLs[this._rId]) window.dhtmlXFormLs[this._rId][a] = null;
			window.dhtmlXFormLs[this._rId] = null;
			try { delete window.dhtmlXFormLs[this._rId]; } catch(e){}
		}
	...
	this._ccTm = null;
	this._ccDo = function() {
		for (var a in window.dhtmlXFormLs) {
			for (var q=0; q<window.dhtmlXFormLs[a].inps.length; q++) {
				var idd = window.dhtmlXFormLs[a].inps[q]._idd;
				if (String(window.dhtmlXFormLs[a].inps[q].tagName).toLowerCase() == "select") {
					var v = "";
					if (window.dhtmlXFormLs[a].inps[q].selectedIndex >= 0 && window.dhtmlXFormLs[a].inps[q].selectedIndex < window.dhtmlXFormLs[a].inps[q].options.length) {
						v = window.dhtmlXFormLs[a].inps[q].options[window.dhtmlXFormLs[a].inps[q].selectedIndex].value;
					}
				} else {
					var v = window.dhtmlXFormLs[a].inps[q].value;
				}
				if (v != window.dhtmlXFormLs[a].vals[idd]) {
					window.dhtmlXFormLs[a].vals[idd] = v;
					window.dhtmlXFormLs[a].form.callEvent("onInputChange",[idd,v,this]);
				}
			}
		}
		if (this._ccTm) window.clearTimeout(this._ccTm);
		this._ccTm = window.setTimeout(function(){that._ccDo();},70);
	}
	...
	doAttachEvents: function(item) {
		...
		if (item._type = "ta") {
			item.childNodes[item._ll?1:0].childNodes[0].onfocus = function() {
				...
				if (i._df != null) {
					this.value = i._value||"";
					window.dhtmlXFormLs[i.getForm()._rId].vals[i._idd] = this.value;
				}
			}
		}
...
if (!window.dhtmlxValidation){
	...
	dhtmlxValidation.prototype = {
		...
		isValidCurrency: function(value) { // Q: Should I consider those signs valid too ? : 垄|鈧瑋鈧鈧楼
